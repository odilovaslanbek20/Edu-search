...
function ProductsDetails() {
  ...
  const { data, isLoading, error } = useGetHooks(`${url}/centers/${id}`)
    ...
    return (
      <div className='max-w-3xl mx-auto p-6'>
        <Alert variant='destructive'>
          <AlertDescription>Markaz topilmadi yoki xatolik yuz berdi.</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    <section className='max-w-5xl mx-auto p-6 mt-10 space-y-10'>
      <Card className='shadow-md border border-gray-200'>
        ...
        <CardHeader>
          <CardTitle className='text-3xl font-bold text-[#461773]'>{center.name}</CardTitle>
        </CardHeader>

        <CardContent className='space-y-2 text-gray-700'>
          <div><strong className='text-[#461773]'>📍 Hudud:</strong> {center.region?.name || "Noma'lum"}</div>
          <div><strong className='text-[#461773]'>📞 Telefon:</strong> {center.phone || "Ko'rsatilmagan"}</div>
          <div><strong className='text-[#461773]'>🏠 Manzil:</strong> {center.address || "Yo'q"}</div>
            ...
            {center.filials.map((filial) => (
              <div key={filial.id} className='flex items-center space-x-4'>
                ...
                <div>
                  ...
                  <p className='text-gray-600'>📍 {filial.region?.name || "Noma'lum"}</p>
            ...
            {center.comments.map((comment) => (
              <div key={comment.id} className='border p-4 rounded-md shadow-sm'>
                <div className='flex items-center space-x-4 mb-2'>
                  ...
                  <div>
                    <p className='font-semibold'>{comment.user ? `${comment.user.firstName} ${comment.user.lastName}` : 'Anonim'}</p>
                  ...
                  <div className='ml-auto text-yellow-500 font-bold'>
                    {'⭐'.repeat(comment.star)}{comment.star < 5 ? '☆'.repeat(5 - comment.star) : ''}
                  </div>
                </div>
